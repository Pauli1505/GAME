	...
	case EV_ITEM_PICKUP:
		...
		}
		{
			...
			if ( item->giType == IT_POWERUP || item->giType == IT_TEAM) {
				...
				trap_S_StartSound (NULL, es->number, CHAN_AUTO,	trap_S_RegisterSound_engine( item->pickup_sound, qfalse ) );
			}
			}
	...
	case EV_GLOBAL_ITEM_PICKUP:
		DEBUGNAME("EV_GLOBAL_ITEM_PICKUP");
		{
			...
			if( item->pickup_sound ) {
				trap_S_StartSound (NULL, cg.snap->ps.clientNum, CHAN_AUTO, trap_S_RegisterSound_engine( item->pickup_sound, qfalse ) );
			}
