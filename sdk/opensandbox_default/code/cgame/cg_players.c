...
sfxHandle_t	CG_CustomSound( int clientNum, const char *soundName ) {
	...
	if ( soundName[0] != '*' ) {
		return trap_S_RegisterSound_engine( soundName, qfalse );
	}
...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName, const char *legsModelName, const char *legsSkinName ) {
	...
	ci->legsModel = trap_R_RegisterModel_engine( filename );
	if ( !ci->legsModel ) {
	...
	ci->torsoModel = trap_R_RegisterModel_engine( filename );
	if ( !ci->torsoModel ) {
		Com_Printf( "Failed to load model file %s\n", filename );
		return qfalse;
	}
	...
	ci->headModel = trap_R_RegisterModel_engine( filename );
	if ( !ci->headModel ) {
		Com_Printf( "Failed to load model file %s\n", filename );
		return qfalse;
	}
...
//	char		redTeam[MAX_QPATH];
	...
	for ( i = 0 ; i < MAX_CUSTOM_SOUNDS ; i++ ) {
		...
		if (modelloaded) {
			ci->sounds[i] = trap_S_RegisterSound_engine( va("sound/player/%s/%s", dir, s + 1), qfalse );
		}
		if ( !ci->sounds[i] ) {
			ci->sounds[i] = trap_S_RegisterSound_engine( va("sound/player/%s/%s", fallback, s + 1), qfalse );
		}
	}
