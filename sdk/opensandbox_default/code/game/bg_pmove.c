...
// CODE POINTERS BECAUSE YES

#define PM_ACCELFL pm_accelerate
// point this to a custom pm_accelerate float for your mod.
...
float	pm_accelerate = 15.0f; // dickhead move
float	pm_airaccelerate = 3.0f;
float	pm_veh00001accelerate = 10.0f*0.090; //VEHICLE-SYSTEM: accelerate control
float	pm_wateraccelerate = 4.0f;
float	pm_flyaccelerate = 8.0f;
float	pm_ladderAccelerate = 50; // what dumbass put this to 3000? thats way to fast

float	pm_friction = 4.0f; // sadly this dumbass engine wont let me define friction for each material defined in the current map so yes
...
==============
*/
static void pm_acceleratefunc( vec3_t wishdir, float wishspeed, float accel ) {
if(mod_accelerate == 1 && !pm->ps->stats[STAT_VEHICLE]) {  //VEHICLE-SYSTEM: disable player accelerate for all
	...
	accelspeed = accel*pml.frametime*wishspeed; // isnt wishspeed our desired speed? this is multiplying the parent variable by your frametime (normally corresponding to the fps)
	// and we are just over here multiplying the speed we actually want 3 times and using that to cap the speed? the fuck???
	...
	canPush = accel*pml.frametime*wishspeed; // just do accelspeed bro
	if (canPush > pushLen) {
		canPush = pushLen;
	}
...
static void PM_WaterMove( void ) {
	...
	pm_acceleratefunc(wishdir, wishspeed, pm_wateraccelerate);
...
static void PM_NoclipMove( void ) {
	...
	pm_acceleratefunc( wishdir, wishspeed, pm_accelerate );
...
static void PM_FlyMove( void ) {
	...
	pm_acceleratefunc(wishdir, wishspeed, pm_flyaccelerate);
...
static void PM_AirMove( void ) {
	...
	pm_acceleratefunc(wishdir, wishspeed, pm_airaccelerate);
...
//	wishvel[2] = 0;
	...
	if ( pm->ps->pm_flags & PMF_DUCKED ) {
		if ( wishspeed > pm->ps->speed * pm_duckScale ) {
			...
			if ( wishspeed < 100 )		// i mean uhh uhhh eh uhh ehh uhh i ah ehh uhh
				wishspeed = 100;
	...
	if ( ( pml.groundTrace.surfaceFlags & SURF_SLICK ) || pm->ps->pm_flags & PMF_TIME_KNOCKBACK || mod_slickmove == 1 ) {
		...
		accelerate = PM_ACCELFL;
	}

	pm_acceleratefunc(wishdir, wishspeed, accelerate);
...
static void PM_LadderMove( void ) {
	...
	pm_acceleratefunc(wishdir, wishspeed, pm_ladderAccelerate);
